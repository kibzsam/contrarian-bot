...
router.get('/', (_req: Request, res: Response) => {
  try {
    const results = Array.from(backtestResults.values()).sort(
      (a, b) => b.startingBudget - a.startingBudget
    );
 ...
 */
router.post('/run-file', async (req: Request, res: Response) => {
  try {
...
router.get('/stats/summary', (_req: Request, res: Response) => {
  try {
    ...
    const rois = results.map((r) =>
      ((r.finalBudget - r.startingBudget) / r.startingBudget) * 100
    );
